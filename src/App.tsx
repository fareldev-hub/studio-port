...
import type { FileSystemEntry, OpenTab, ContextMenuItem } from '@/types';
...
export default function App() {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.key === 'F2' && selectedEntry) {
        e.preventDefault();
        setRenamingEntry({ entry: selectedEntry, newName: selectedEntry.name });
      }
      if (e.key === 'Delete' && selectedEntry) {
        e.preventDefault();
        setDeleteModal({ entry: selectedEntry });
      }
    };
      ...
      const updateEntryChildren = (entries: FileSystemEntry[]): FileSystemEntry[] => {
        return entries.map((e) => {
          if (e.path === entry.path) {
            return { ...e, expanded: true, children };
          }
          if (e.children) {
            return { ...e, children: updateEntryChildren(e.children) };
          }
          return e;
        });
      };
  ...
  const handleCreateItem = useCallback(
    async (name: string) => {
      ...
      if (type === 'file') {
        ...
        if (handle) {
          toast.showInfo('File Created', `"${name}" has been created.`);
          await fs.refreshEntries();
        } else {
          toast.showError('Failed to create file.');
        }
      } else {
        const handle = await fs.createDirectory(parentHandle, name);
        if (handle) {
          toast.showInfo('Folder Created', `"${name}" has been created.`);
          await fs.refreshEntries();
        } else {
      ...
      if (success) {
        ...
        if (tab) {
          setTabs((prev) =>
            prev.map((t) =>
              t.id === tab.id
                ? { ...t, name: newName, path: entry.path.replace(entry.name, newName) }
                : t
            )
          );
        }
  ...
  useEffect(() => {
    const handleMouseMove = (e: MouseEvent) => {
      if (sidebarResizeRef.current.isResizing) {
        ...
        const newWidth = Math.max(180, Math.min(500, sidebarResizeRef.current.startWidth + delta));
        setSidebarWidth(newWidth);
      }
      if (terminalResizeRef.current.isResizing) {
        const delta = terminalResizeRef.current.startY - e.clientY;
        const newHeight = Math.max(100, Math.min(window.innerHeight * 0.6, terminalResizeRef.current.startHeight + delta));
      ...
      const items: ContextMenuItem[] = [
        {
          ...
          action: () => setNewItemModal({ type: 'file', parentHandle: entry.kind === 'directory' ? (entry.handle as FileSystemDirectoryHandle) : (entry.parentHandle as FileSystemDirectoryHandle) }),
        },
        {
          label: 'New Folder',
          icon: 'fa-folder-plus',
          action: () => setNewItemModal({ type: 'folder', parentHandle: entry.kind === 'directory' ? (entry.handle as FileSystemDirectoryHandle) : (entry.parentHandle as FileSystemDirectoryHandle) }),
        },
        { label: '', divider: true, action: () => {} },
        {
          ...
          icon: 'fa-pen',
          shortcut: 'F2',
          action: () => setRenamingEntry({ entry, newName: entry.name }),
        },
        {
          label: 'Delete',
          icon: 'fa-trash-can',
          shortcut: 'Del',
          action: () => setDeleteModal({ entry }),
        },
        { label: '', divider: true, action: () => {} },
        {
          label: 'Refresh',
          icon: 'fa-rotate-right',
          action: () => fs.refreshEntries(),
        },
      ];
  ...
  return (
    <div className="flex flex-col w-screen h-screen overflow-hidden" style={{ backgroundColor: 'var(--bg-primary)' }}>
      ...
      {deleteModal && (
        <DeleteConfirmModal
          fileName={deleteModal.entry.name}
